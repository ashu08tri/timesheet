...
type FilterStatus = 'all' | 'SUBMITTED' | 'IN_REVIEW' | 'APPROVED' | 'REVERTED'
...
export default function ApprovalsPage() {
  ...
  const [onlyMineReverted, setOnlyMineReverted] = useState(true)
  ...
  async function reject() {
    ...
    try {
      const res = await fetch(`/api/timesheets/${rejectModal.id}/revert`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ notes: rejectNotes }),
      })
      if (!res.ok) throw new Error()
      toast.success('Timesheet reverted with feedback')
      setRejectModal(null)
      setRejectNotes('')
      await load()
    } catch {
  ...
  ).filter(t => !(filter === 'REVERTED' && onlyMineReverted) || t.reviewer?.id === session?.user.id)

  const counts = {
    SUBMITTED: timesheets.filter(t => t.status === 'SUBMITTED').length,
    IN_REVIEW: timesheets.filter(t => t.status === 'IN_REVIEW').length,
    APPROVED:  timesheets.filter(t => t.status === 'APPROVED').length,
    REJECTED:  timesheets.filter(t => t.status === 'REVERTED').length,
  }
    ...
    <>
      ...
      <div className="flex items-center gap-1 p-1 bg-gray-100 dark:bg-gray-800 rounded-xl mb-6 w-fit overflow-x-auto">
        {(
          [
            ...
            { key: 'REVERTED',  label: 'Reverted',  count: counts.REJECTED  },
      ...
      {filter === 'REVERTED' && (
        <label className="flex items-center gap-2 mb-4 text-xs text-gray-500">
          <input type="checkbox" checked={onlyMineReverted} onChange={e => setOnlyMineReverted(e.target.checked)} />
          Only entries I reverted
        </label>
      )}
                  ...
                  {expanded === ts.id && (
                    ...
                    >
                      <div className="p-4 space-y-2 bg-gray-50/50 dark:bg-gray-800/30">
                        ...
                        {ts.status === 'REVERTED' && (
                          <div className="mt-2 p-3 bg-red-50 dark:bg-red-950/20 border border-red-100 dark:border-red-900 rounded-xl">
                            <p className="text-xs font-semibold text-red-600 dark:text-red-400 mb-1">
                              Reverted by {ts.reviewer?.name ?? 'reviewer'}
                              {ts.reviewedAt && ` · ${format(new Date(ts.reviewedAt), 'MMM d, yyyy')}`}
                            </p>
                            <p className="text-sm text-red-700 dark:text-red-300">{ts.reviewNotes || 'No comments provided.'}</p>
                          </div>
                        )}
                        {ts.notes && (
      ...
      >
        <div className="space-y-4">
          <div>
            <label className="label">Comments for revert *</label>
            <textarea
              value={rejectNotes}
